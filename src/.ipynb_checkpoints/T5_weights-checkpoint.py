...
from .datasethp import HotpotDataset, my_collate
...
#                     print("batch keys: ", batch.keys()) batch keys:  dict_keys(['input_ids', 'labels'])
                    ...
                    outputs = self.model(input_ids=source_ids,
                                         ...
                                         return_dict=True, )
                    print("############## outputs['decoder_attentions']", outputs['decoder_attentions'][0].shape)
                    print(outputs['decoder_attentions'][0])
                    print("------------------------------------------------------------------------------------")
...
#             print("------------------  HotpotQA  -------------------------")
            ...
            answer_dict = {}  # from generate
            answer_dict2 = {}  # from generate
            answer_dict3 = {}  # from generate
            answer_dict_forward = {}  # from generate
            ...
            def f1_score_(prediction, ground_truth):
...
#                 print("normalized_ground_truth: ", normalized_ground_truth)
    ...
    #                 return ZERO_METRIC
            ...
            def update_answer(metrics, prediction, gold):
                ...
                if em:
                    print("exact_match_score: prediction, gold>>>", prediction," : ", gold,"|")

                f1, prec, recall = f1_score_(prediction, gold)
                metrics['em'] += float(em)
                metrics['f1'] += f1
                metrics['prec'] += prec
            ...
            with torch.no_grad():
                for batch in tqdm(test_dataloader):
                    #print("batch keys=====================> ", batch.keys()) # dict_keys(['source', 'source_ids', 'source_mask', 'target', 'target_ids', 'target_mask', 'source_sents', 'target_sents', 'q_id'])
                    ...
                    target = batch["target"]
                    facts = batch['facts']
                    source_sents, target_sents = batch['source_sents'], batch['target_sents']
                    decoded = []
                    outputs = self.model(input_ids=source_ids,
                    ...
                    ground_answer = dict(zip(batch['q_id'], decoded_ground))
                    ground_dict.update(ground_answer)   # references
                    ground_answer_txt = dict(zip(batch['q_id'], target))
                    ground_dict_text.update(ground_answer_txt)
                    fact_ = dict(zip(batch['q_id'], facts))
                    facts_dict.update(fact_)
                    
                    hp_answer = self.model.generate(source_ids)  
                    hotpot_answer1 = [
                        self.tokenizer.decode(
                                  answer, skip_special_tokens=True, clean_up_tokenization_spaces=False
                            )
                            for answer in hp_answer
                        ]
                    print("1: decoded_answer using self.model.generate(source_ids) -->> ")
                    print(hotpot_answer1)
                    print("********************************************************")

                    hp_answer = self.model.generate(input_ids=source_ids,  # Is not used in metrics
                                                        eos_token_id=self.tokenizer.eos_token_id,
                                                        do_sample=True,
                                                        early_stopping=True, top_p=0.9, top_k=30,
                                                        length_penalty=1.5,
                    ...
                    print("2: decoded_answer using self.model.generate(input_ids=source_ids,  eos_token_id=self.tokenizer.eos_token_id, do_sample=True, early_stopping=True, top_p=0.9, top_k=30,length_penalty=1.5,temperature=0.9, repetition_penalty=2.0) -->> ")
                    answer_dict_ = dict(zip(batch['q_id'], hotpot_answer2))  # answers of the model
                    answer_dict2.update(answer_dict_)
                    for key, value in answer_dict2.items():
                        print(key, ": ", value)
                    print("*************************************************************************")
                    ...
                    print("3: decoded_answer using self.model.generate(input_ids=source_ids, eos_token_id=self.tokenizer.eos_token_id, do_sample=True, early_stopping=True, top_k=40, length_penalty=2.0, emperature=0.5, repetition_penalty=2.0)-->> ")
                    answer_dict_ = dict(zip(batch['q_id'], hotpot_answer3))  # answers of the model
                    answer_dict3.update(answer_dict_)
                    for key, value in answer_dict3.items():
                        print(key, ": ", value)
                    print("*************************************************************************")
                    ...
                    sents = [[self.tokenizer.convert_ids_to_tokens(t_id).replace('▁', '') for t_id in
                              sent.tolist()] for sent in sentences]
                    sents = [tokens[:self.get_actual_seq_len(tokens)] for tokens in sents]

                    max_logits = torch.argmax(outputs['logits'], dim=-1) # max_logits shape:  torch.Size([20, 40]) batch_size, target_length

                    model_output = [self.tokenizer.convert_ids_to_tokens(t_id).replace('▁', '#') for t_id in
                              max_logits[0].tolist()]
                    decoded_model_output = [[self.tokenizer.convert_ids_to_tokens(t_id).replace('▁', '#') for t_id in  # output of the mosel resulted from using forward
                              max_logit.tolist()] for max_logit in max_logits]
    
                    decoded_logits = [self.tokenizer.decode(
                                  answer, skip_special_tokens=True, clean_up_tokenization_spaces=True)                           
                            for answer in max_logits]
                    answer_dict_forward_ = dict(zip(batch['q_id'], decoded_logits))  # answers of the model
                    answer_dict_forward.update(answer_dict_forward_)
                
                    print("*************************************************************************")
                    print("answer_dict_forward: ")
                    for key, value in answer_dict_forward.items():
                        print(key, ": ", value)
                    print("***********************facts_dict*************************************")
                    ...
                    y_tokens = [self.tokenizer.convert_ids_to_tokens(t_id).replace('▁', '#') for t_id in
                                self._shift_right(target_ids[0]).tolist()]   #  reference 
        
                    ground_selector_tokens = [[self.tokenizer.convert_ids_to_tokens(t_id).replace('▁', '#') for t_id in
                                self._shift_right(target_id).tolist()]for target_id in target_ids ]
                    decoder_input_tokens = [tokens[:self.get_actual_seq_len(tokens)] for tokens in ground_selector_tokens]
                    ...
                    print("************************************************************")
                    keys = sents
                    for key, prediction in answer_dict_forward.items():
                        update_answer(metrics, prediction, ground_dict[key])
                    print("#################################")
                    print("1: metrics before normalizing:")
                    print(metrics)
                    print("#################################")
                    N = len(answer_dict_forward.keys())
                    print("len(metrics['f1']):    ", N)
                    for k in metrics.keys():
                        metrics[k] /= N 
                        
                    print("#################################")
                    print("2: metrics after normalizing:")
                    print(metrics)
                    print("#################################")
                    return outputs['encoder_attentions'], outputs['cross_attentions'], outputs['decoder_attentions'] ,keys, ground_selector_tokens, decoder_input_tokens, source_sents, target_sents, queries, decoded_model_output, facts  
...
#             print("------------------------  MLM  -------------------------------")
           ...
           ####### encoder_attentions, cross_attentions, decoder_attentions , selector_attention, tokens, y_tokens, sents = main()
            ...
            print("-------------------  Hotpot QA ---------------------------")
            test_dataloader = self._create_dataloders()
            encoder_attentions, cross_attentions, decoder_attentions , keys, ground_selector_tokens, decoder_input_tokens, source_sents, target_sents, queries, decoded_model_output, facts = self.test(test_dataloader)
           ####### encoder_attentions, cross_attentions, decoder_attentions , selector_attention, tokens, y_tokens, sents = main()
            return encoder_attentions, cross_attentions, decoder_attentions , keys, ground_selector_tokens, decoder_input_tokens, source_sents, target_sents, queries, decoded_model_output, facts
